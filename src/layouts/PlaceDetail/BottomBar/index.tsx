...
const BottomBar = ({ name, x, y }: Props) => {
  ...
  const LinkSearchRoad = () => {
    window.location.href = `http://m.map.naver.com/route.nhn?menu=route&sname=내위치&sx=${userLocation.x}&sy=${userLocation.y}&ename=${name}&ex=${x}&ey=${y}&pathType=0&showMap=true`;
    //네이버 앱으로 길찾기 실행
